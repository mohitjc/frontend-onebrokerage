  ...
  return (
    <Layout>
      ...
      {/* <div className="table-responsive table_section">
        <table className="table table-stripeds table-bordered planTable">
                ...
                return <>
                  <th scope="col" className='table_data'>
                    <div className='tableHeading text-center'>
                      ...
                      {item.id == activeplan?.planId?._id && activeplan?.isActive ? <>
                        {activeplan?.on_trial ? <p className='text-primary mb-0'>Free Trial<br /></p> : null}
                        Valid Upto : <span className='text-primary'>{datepipeModel.date(activeplan?.validUpTo)}</span><br />
                        {activeplan?.isActive && activeplan?.planInterval == interval ? <>
                          <button className="btn btn-primary mt-1" onClick={e => cancelplan(item.id)}>Cancel Subscription</button>
                        </> : <><button className="btn btn-primary mt-1" onClick={e => getplandetails(item)}>{activeplan?.isActive ? 'Upgrade' : 'Get Started'}</button></>}
                      </> : <>
                        {!user?.on_trial && !user?.trial_ended && item?.trial_period_days ? <p className='text-primary mb-0'>{item?.trial_period_days} Days Free Trail</p> : null}
                        <button className='btn btn-primary w-100 mt-4' disabled={getPrice(item) ? false : true} onClick={e => getplandetails(item)}>{activeplan?.isActive ? 'Upgrade' : 'Get Started'}</button>
                      </>}
                ...
                {features[oitm].map(fitm => {
                  return <>
                    <tr>
                      <td>{fitm.name}
                        {fitm.description ? <>
                          <a id={`app-title_${fitm.id}`} className='borderI'><i className="fa fa-info" aria-hidden="true"></i></a>
                          <ReactTooltip
          ...
          {data &&
            data.map((item, index) => {
              return (
                <>
                  <div>
                    <div className="shadow-box !rounded-xl !p-5">
                                    ...
                                    return (
                                      <li>
                                        <div className="flex items-center gap-1.5 text-[#3C3E49] capitalize">
                                          {isChecked(item, fitm) ? (
                                            <IoCheckmarkCircle className="text-xl !text-primary" />
                                          ) : (
